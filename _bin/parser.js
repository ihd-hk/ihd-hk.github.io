...
data.items = data.items.filter(function(item) {
  return (item.project !== "0") && !!(item.project) && (item.nda !== 'Y') && (item.title_en !== "0") && (item.country_en !== "0");
}).map(function(item, index, source) {
  item['disciplines'] = Array.from(item['disciplines']);
  item['codes'] = Array.from(item['codes']);
  ['desc_en', 'desc_ch'].forEach(function(attribute) {
    item[attribute] = item[attribute] == "0" ? '' : item[attribute];
    item[attribute] = item[attribute] == '' ? null : item[attribute];
  })
  return item;
}).sort(function(a, b) {
